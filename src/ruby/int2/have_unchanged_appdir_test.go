...
func (matcher *haveUnchangedAppdirMatcher) Match(actual interface{}) (success bool, err error) {
	app, ok := actual.(models.App)
	if !ok {
		return false, fmt.Errorf("HaveUnchangedAppdir matcher requires a cfapi.models.App.  Got:\n%s", format.Object(actual, 1))
	}

	re := regexp.MustCompile(fmt.Sprintf("%s: ([0-9a-f]+)", matcher.before))
	matches := re.FindAllStringSubmatch(app.Log(), -1)
...
func (matcher *haveUnchangedAppdirMatcher) FailureMessage(actual interface{}) (message string) {
	app := actual.(models.App)
	if matcher.checksumBefore == "" {
		return format.Message(app.Log(), "to contain substring", matcher.before)
	} else if matcher.checksumAfter == "" {
		return format.Message(app.Log(), "to contain substring", matcher.after)
	}
